#!/usr/bin/env python3
"""Train 6-input manual climate ANN for on-device Android form inference."""
...
def build_manual_climate_frame(df: pd.DataFrame, start: int, end: int) -> pd.DataFrame:
    rows = []
    for _, row in df.iterrows():
        rows.append({
            ...
            "season_wind_mean": season_mean(row, "wind_speed", start, end),
            "season_solar_mean": season_mean(row, "solar_radiation", start, end),
            "season_humidity_mean": season_mean(row, "relative_humidity", start, end),
            "yield_t_ha": float(row["Yield Level (t/ha)"]),
        })
    out = pd.DataFrame(rows)
    for col in MANUAL_CLIMATE_FEATURES:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train 6-input manual climate ANN.")
    parser.add_argument(
        "--input",
    ...
    parser.add_argument(
        "--output",
        default="app/src/main/assets/ml/yield_models_manual_climate_ann.json",
    )
    ...
    frame = build_manual_climate_frame(df, args.start, args.end)
    X = frame[MANUAL_CLIMATE_FEATURES].values
    ...
    payload = {
        ...
        "feature_names": MANUAL_CLIMATE_FEATURES,
        "predictor_labels": {
            "season_rainfall_mm": "Rain (mm)",
            "season_t_min_mean": "T min (°C)",
            "season_t_max_mean": "T max (°C)",
            "season_wind_mean": "Wind (m/s)",
            "season_solar_mean": "Solar (kWh/m²/day)",
        ...
        "defaults": {
            ...
            "median_season_wind_mean": round(float(frame["season_wind_mean"].median()), 2),
            "median_season_solar_mean": round(float(frame["season_solar_mean"].median()), 2),
            "median_season_humidity_mean": round(float(frame["season_humidity_mean"].median()), 2),
        },
        "ann": {
            "metrics": ann_metrics,
    ...
    print(f"Saved: {out}")
    print("Medians:", payload["defaults"])
